...
class MuCalculator(nn.Module):
    def forward(self, in_vars: Dict[str, Tensor]) -> Dict[str, Tensor]:
        return {"mu2_out": torch.abs(in_vars["alpha"]-1.0)*in_vars["mu2"]}
...
def run(cfg: ModulusConfig) -> None:
    ...
    nodes = (ns.make_nodes() + slurry_viscosity.make_nodes() 
             ...
             #+ [Node(['alpha','mu2'], ['mu2_out'], MuCalculator())] 
             #+ [Node.from_sympy(Abs(Symbol("alpha")-1.0)*Symbol("mu2"), "mu2_out")]
    ...
    integral_continuity = IntegralBoundaryConstraint(
        ...
        outvar={"normal_dot_vel": -1*v_in*Lf},
        batch_size=3,
        integral_batch_size=cfg.batch_size.integral_continuity,
        lambda_weighting={"normal_dot_vel": 1.0},
        #parameterization={Symbol("t"): (0, time_window_size), Symbol("x"): (right_rx,(Lf+right_width))}
