...
def run(cfg: ModulusConfig) -> None:
    ...
    def importance_measure(invar):
        ...
        importance = 2*(0.5-torch.abs(0.5-outvar["a"]))
        #importance = (
        #    outvar["a__x"] ** 2
        #    + outvar["a__y"] ** 2
        #) ** 0.5 * 1000
    ...
    ic_slurry_up = PointwiseInteriorConstraint(
        ...
        outvar={
            "u": 0,
            "v": -1*v_in,
            "p": 20-0.66*(mid_height+H0-y)/H0,
            "a": 0,
        },
        batch_size=cfg.batch_size.initial_condition,
        lambda_weighting={"u": Symbol("sdf"), "v": Symbol("sdf"), "p": 100, "a": 100},
        criteria=And((x > 0.0), (x < Lf), (y>H0)),
        parameterization={t_symbol: 0},
    )
    ic_domain.add_constraint(ic_slurry_up, name="ic_slurry_up")       

    ic_slurry_mid = PointwiseInteriorConstraint(
        nodes=nodes,
        geometry=geo_coating,
        outvar={
            "u": 0,
    ...
    ic_domain.add_constraint(ic_slurry_mid, name="ic_slurry_mid")       
    ...
    inlet = PointwiseBoundaryConstraint(
        nodes=nodes,
        geometry=geo,
        outvar={"u": 0, "v": -1*v_in, "a": 0, "p": 20.0},
