...
class NavierStokes_VOF(PDE):
    ...
    def __init__(self, mus, rhos, sigma, g, U_ref,L_ref, dim=2, time=True, mixed_form=False):
        ...
        abs_interface_grad = ((a.diff(x))**2 + (a.diff(y))**2 + np.finfo(float).eps)**(0.5)
        curvature = - ( (a.diff(x).diff(x) + a.diff(y).diff(y))/abs_interface_grad - (a.diff(x)**2*a.diff(x).diff(x) + a.diff(y)**2*a.diff(y).diff(y) + 2*a.diff(x)*a.diff(y)*a.diff(x).diff(y))/(abs_interface_grad)**3 )
