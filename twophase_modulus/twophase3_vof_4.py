...
Desc: twophase 3-3
- Twop3-3: boundary moving 추가 "v":0.3/L_ref
Desc: twophase 3-4
- 3-3 + adaptive ac., no normalize
'''

@modulus.main(config_path="conf", config_name="config_twophase3")
def run(cfg: ModulusConfig) -> None:
    ...
    highres_width = (-0.5, 0.5)
    highres_length = (-0.25, 0.75)
    ...
    interface = PointwiseBoundaryConstraint(
        nodes=nodes,
        geometry=bubble,
        outvar={"PDE_m": 0, "PDE_a": 0, "PDE_u": 0, "PDE_v": 0,"v":0.3},
        #bounds=box_bounds,
        batch_size=cfg.batch_size.interface,
        lambda_weighting={"PDE_m": 1.0, "PDE_a": 1.0,   "PDE_u": 10.0, "PDE_v": 10.0,"v":1.0},
        parameterization=time_range,
    )
